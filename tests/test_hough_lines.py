...
from docdetect.hough_lines import detect_lines, _group_similar, _cvhoughlines2list, _is_duplicated
...
class TestCannyEdges(TestCase):
    ...
    def test_detects_correct_format(self):
        self.assert_format_is_correct(detect_lines(self.edges, group_similar_thr=30))
        self.assert_format_is_correct(detect_lines(self.edges, group_similar_thr=30))
    ...
    def test_detects_group_similar_reduces_number_of_lines(self):
        raws = detect_lines(self.edges, group_similar_thr=0)
        groupeds = detect_lines(self.edges, group_similar_thr=30)
    ...
    def test_is_duplicated(self):
        self.assertTrue(_is_duplicated((0, 0), [(0, 0)], 1))
        self.assertFalse(_is_duplicated((0, 0), [(1, 0)], 1))
        self.assertTrue(_is_duplicated((0, 0), [(1, 0)], 2))
        self.assertFalse(_is_duplicated((0, 0), [], 1))
